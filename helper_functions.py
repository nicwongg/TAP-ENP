...
def add_new_shortened_url(shortened, url):
    conn = get_db_connection()
    conn.execute(f"""INSERT INTO urls (shorten_id, original_url) VALUES (?, ?);""", (shortened, url))
    conn.commit()
    conn.close()
...
def check_alias_exist(custom_alias):
    conn = get_db_connection()
    res = conn.execute(f"SELECT * from urls where shorten_id = '{custom_alias}'").fetchall()
    conn.close()
    return len(res) > 0

def get_original_url(custom_alias):
    conn = get_db_connection()
    url = conn.execute(f"SELECT original_url from urls where shorten_id = '{custom_alias}'").fetchone()
    conn.close()
    return url
